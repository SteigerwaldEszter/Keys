...
class KeyLog(db.Model):
    ...
    signature_id: Mapped[int] = mapped_column(Integer, ForeignKey("signatures.id"), nullable=False)
    ...
    handler = relationship("User", foreign_keys=[handler_id], back_populates="handled_key_logs")
    receiver = relationship("User", foreign_keys=[receiver_id], back_populates="received_key_logs")
